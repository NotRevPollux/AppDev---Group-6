document.addEventListener('DOMContentLoaded', function() {
  ...
  const supplyConsumption = {
    'regular_laundry': { detergent: 1, softener: 1, bleach: 1, fragrance: 1, stain_remover: 1, steam_water: 0, garment_bag: 0 },
    'wash_and_fold': { detergent: 1, softener: 1, bleach: 1, fragrance: 1, stain_remover: 1, steam_water: 0, garment_bag: 1 },
    'dry_cleaning': { detergent: 0, softener: 0, bleach: 0, fragrance: 1, stain_remover: 1, steam_water: 0, garment_bag: 1 },
    'iron_and_press': { detergent: 0, softener: 0, bleach: 0, fragrance: 1, stain_remover: 0, steam_water: 1, garment_bag: 0 }
  };
  ...
  function fetchOrdersFromBackend(callback) {
    fetch('/aquaruse/accounts_backend.php?action=get_orders')
      .then(response => response.json())
      .then(data => callback(data))
      .catch(() => callback([]));
  }

  function loadOrdersAndRender() {
    fetchOrdersFromBackend(function(orders) {
      dashboardOrders = orders.map(order => ({
        name: order.name,
        number: order.number,
      ...
      // Set orderIdCounter to max existing orderId + 1
      let maxId = 0;
      dashboardOrders.forEach(o => {
        const idNum = parseInt(o.orderId, 10);
  ...
  if (modalForm) {
    modalForm.addEventListener('submit', function(e) {
      ...
      if (editingOrderIndex !== null) {
        const orderId = dashboardOrders[editingOrderIndex].orderId;
        fetch('/aquaruse/accounts_backend.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: `action=update_order&order_id=${encodeURIComponent(orderId)}&date=${encodeURIComponent(dateValue)}&service_type=${encodeURIComponent(service)}&kg=${encodeURIComponent(load)}&total_amount=${encodeURIComponent(amount)}&amount_paid=${encodeURIComponent(paid)}&balance=${encodeURIComponent(balance)}&status=${encodeURIComponent(statusValue)}&customer_name=${encodeURIComponent(name)}&number=${encodeURIComponent(number)}`
        }).then(() => {
          editingOrderIndex = null;
          loadOrdersAndRender();
          modalForm.reset();
      ...
      const consumption = supplyConsumption[service.toLowerCase().replace(/ /g, '_').replace(/__/g, '_')];
      let canDeduct = true;
      ...
      fetch('/aquaruse/accounts_backend.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `action=add_order&order_id=${encodeURIComponent(orderId)}&date=${encodeURIComponent(dateValue)}&service_type=${encodeURIComponent(service)}&kg=${encodeURIComponent(load)}&total_amount=${encodeURIComponent(amount)}&amount_paid=${encodeURIComponent(paid)}&balance=${encodeURIComponent(balance)}&status=${encodeURIComponent(statusValue)}&customer_name=${encodeURIComponent(name)}&number=${encodeURIComponent(number)}`
      }).then(() => {
        loadOrdersAndRender();
      });
      let customerIndex = customers.findIndex(c => c.name === name && c.number === number);
      if (customerIndex !== -1) {
        customers[customerIndex].type = 'Returning';
      } else {
  ...
  if (modalDeleteBtn) {
    modalDeleteBtn.addEventListener('click', function() {
      if (editingOrderIndex !== null) {
        const orderIdToDelete = dashboardOrders[editingOrderIndex].orderId;
        fetch('/aquaruse/accounts_backend.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: `action=delete_order&order_id=${encodeURIComponent(orderIdToDelete)}`
        }).then(() => {
          editingOrderIndex = null;
          modalOverlay.classList.add('hidden');
          loadOrdersAndRender();
  ...
  function fetchCustomersFromBackend(callback) {
    fetch('/aquaruse/accounts_backend.php?action=get_customers')
      .then(response => response.json())
      .then(data => callback(data))
      .catch(() => callback([]));
  }

  function loadCustomersAndRender() {
    fetchCustomersFromBackend(function(customers) {
  ...
  function renderCustomersSection(customers) {
    ...
    customers.forEach((customer) => {
      ...
      row.innerHTML = `
        ...
        <td>${customer.is_returning == 1 ? 'Returning' : 'New'}</td>
      `;
  ...
  function fetchSuppliesFromBackend(callback) {
    fetch('/aquaruse/accounts_backend.php?action=get_supplies')
      .then(response => response.json())
      .then(data => {
        supplies = {};
        data.forEach(item => {
          // Convert name to lowercase and replace spaces/underscores for key
          let key = item.name.toLowerCase().replace(/ /g, '_').replace(/__/g, '_');
          supplies[key] = parseInt(item.quantity, 10);
        });
        callback();
      })
      .catch(() => callback());
  ...
  function renderDashboardSupplyList() {
    ...
    Object.keys(supplies).forEach(key => {
      ...
      row.innerHTML = `
        ${supplyLabels[key] || key} <span class="supply-status ${status.class}">${status.text}</span>
      `;
  ...
  function renderSuppliesSection() {
    ...
    Object.keys(supplies).forEach(key => {
      ...
      const status = getStockStatus(supplies[key]);
      row.innerHTML = `
        <td>${supplyLabels[key] || key}</td>
        <td><span id="qty-${key}">${supplies[key]}</span></td>
        <td><span class="supply-status ${status.class}">${status.text}</span></td>
        <td>
          <input type="number" min="0" id="input-${key}" value="${supplies[key]}" style="width:60px; padding:4px; border-radius:4px; border:1px solid #ccc; text-align:center;" />
        </td>
      `;
      tbody.appendChild(row);
    });
      ...
      if (input) {
        input.addEventListener('change', function() {
          ...
          fetch('/aquaruse/accounts_backend.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `action=update_supply_quantity&name=${encodeURIComponent(supplyLabels[key])}&quantity=${encodeURIComponent(val)}`
          })
          ...
          .then(result => {
            if (result === 'success') {
              ...
              input.value = supplies[key];
              alert('Failed to update supply in database.');
            }
          })
          .catch(() => {
            input.value = supplies[key];
  ...
  function renderStaffSuppliesSection() {
    const staffSuppliesList = document.getElementById('staff-supplies-list');
    staffSuppliesList.innerHTML = '';
    const table = document.createElement('table');
    table.className = 'supplies-table';
    const thead = document.createElement('thead');
    thead.innerHTML = `
    ...
    Object.keys(supplies).forEach(key => {
      ...
      row.innerHTML = `
        <td>${supplyLabels[key] || key}</td>
        <td><span id="staff-qty-${key}">${supplies[key]}</span></td>
        <td><span class="supply-status ${status.class}">${status.text}</span></td>
        <td>
          <input type="number" min="0" id="staff-input-${key}" value="${supplies[key]}" style="width:60px; padding:4px; border-radius:4px; border:1px solid #ccc; text-align:center;" />
        </td>
      `;
      tbody.appendChild(row);
    });
    table.appendChild(tbody);
    staffSuppliesList.appendChild(table);
    Object.keys(supplies).forEach(key => {
      const input = document.getElementById(`staff-input-${key}`);
      if (input) {
        input.addEventListener('change', function() {
          let val = parseInt(input.value, 10);
          if (isNaN(val) || val < 0) val = 0;
          supplies[key] = val;
          renderStaffSuppliesSection();
          renderDashboardSupplyList();
        });
      }
    });
  ...
  fetchSuppliesFromBackend(function() {
    renderDashboardSupplyList();
    loadOrdersAndRender();
  });
  ...
  function renderStaffSection() {
    staffList.innerHTML = '';
    // Only show logout or other relevant content, no staff name input
  }
  ...
  function getStaffAccountsFromBackend(callback) {
    fetch('/aquaruse/accounts_backend.php')
      .then(response => response.json())
      .then(data => callback(data))
      .catch(() => callback([]));
  ...
  function renderAccountsSection() {
    ...
    if (!container) return;
    getStaffAccountsFromBackend(function(staffAccounts) {
      staffAccounts = staffAccounts.filter(acc => acc.email !== 'admin@aquaruse');
      let html = `<table class="customers-table"><thead><tr><th>Staff Email</th><th>Staff Name</th><th>Password</th><th>Action</th></tr></thead><tbody id="staff-table-body">`;
      for (const acc of staffAccounts) {
        html += `<tr><td>${acc.email}</td><td>${acc.account_name || ''}</td><td>${acc.password || ''}</td><td><button class='delete-staff-btn' data-id='${acc.id}' style='color:#fff;background:#e53935;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;'>Delete</button></td></tr>`;
      }
      html += `</tbody></table>`;
      html += `
        <form id="add-staff-form" style="margin-top:24px;display:flex;gap:12px;align-items:center;justify-content:center;">
          ...
          <input type="text" id="newStaffEmail" placeholder="Enter staff email" style="padding:8px 12px;border-radius:6px;border:1px solid #ccc;font-size:1rem;" required />
      ...
      if (form) {
        form.onsubmit = function(e) {
          e.preventDefault();
          let account_name = document.getElementById('newStaffName').value.trim();
          ...
          if (!email || !password || !account_name) return;
          fetch('/aquaruse/accounts_backend.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `action=add&account_name=${encodeURIComponent(account_name)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`
          })
          .then(response => response.text())
          .then(result => {
            if (result === 'success') {
